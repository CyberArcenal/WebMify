...
export default class SkillsPage {
  ...
  async fetchSkills() {
    try {
      const response = await apiClient.get(
        "/api/v1/portfolio/skills/?page=1&page_size=50",
      );

      if (response.data?.status) {
        this.featuredSkills = response.data.results.filter(
          (skill) => skill.featured,
        );
        this.additionalSkills = response.data.results.filter(
          (skill) => !skill.featured,
        );
      } else {
  ...
  createAdditionalSkillElement(skill) {
    const skillEl = document.createElement("div");
    skillEl.className =
      "px-5 py-3 bg-white dark:bg-gray-700 rounded-lg shadow flex items-center";
  ...
  toggleAdditionalSkillsSection() {
    ...
    if (section) {
      section.style.display =
        this.additionalSkills.length > 0 ? "block" : "none";
    }
  }
